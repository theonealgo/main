...
export default function Header() {
  ...
  const toggleMenu = (menu: string) => setOpenMenu(openMenu === menu ? '' : menu);

  return (
    <header className="w-full bg-black text-white px-6 py-4 flex items-center justify-between border-b border-gray-800">
      {/* Logo */}
      <Link href="/" className="flex items-center">
        <Image src="/images/theonelogo.png" alt="Logo" width={128} height={128} className="h-10 w-auto" />
      </Link>
      ...
      <nav className="flex items-center gap-6 relative">
        ...
        <div className="relative">
          <button onClick={() => toggleMenu('product')} className="hover:text-teal-400 transition">Product</button>
          {openMenu === 'product' && (
        ...
        {/* Resources */}
        <div className="relative">
          <button onClick={() => toggleMenu('resources')} className="hover:text-teal-400 transition">Resources</button>
          {openMenu === 'resources' && (
            <div className="absolute left-0 mt-2 w-40 bg-gray-900 rounded shadow-lg py-2 z-50">
        ...
        {/* Company */}
        <div className="relative">
          <button onClick={() => toggleMenu('company')} className="hover:text-teal-400 transition">Company</button>
          {openMenu === 'company' && (
            <div className="absolute left-0 mt-2 w-40 bg-gray-900 rounded shadow-lg py-2 z-50">
