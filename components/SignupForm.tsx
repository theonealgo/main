...
function SignupForm({ plan, billing }: { plan: string; billing: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setRedirectUrl(url); // Store the redirect URL
  ...
  useEffect(() => {
    if (redirectUrl) {
      window.location.href = redirectUrl; // Perform the redirect in the client
    }
  }, [redirectUrl]);
