...
export default function AuthClientPage() {
  ...
  const initialView = hint === 'login' ? 'login'
                     : hint === 'forgot' ? 'forgot'
                     : 'signup';
  ...
  async function handleSignup(e: FormEvent) {
    ...
    setError('');
    try {
      // 1) Create account via your signup API
      ...
      const saveRes = await fetch('/api/save-user',{
        method: 'POST',
        headers:{ 'Content-Type':'application/json' },
        body: JSON.stringify({ tradingViewUsername: tvUser, email })
      });
      if (!saveRes.ok) throw new Error('Saving TV username failed.');
