...
function LoginForm() {
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); signIn('credentials', { email, password, callbackUrl: '/dashboard' }); }} className="space-y-4 bg-gray-900 p-6 rounded-lg">
      ...
      <button className="w-full bg-blue-600 py-3 rounded">Log In</button>
    </form>
  );
}
...
function ForgotPasswordForm() {
  const [email, setEmail] = useState('');
  return (
    <form onSubmit={e => { e.preventDefault(); alert(`Reset link sent to ${email}`); }} className="space-y-4 bg-gray-900 p-6 rounded-lg">
      <input type="email" placeholder="Your email" required value={email} onChange={e => setEmail(e.target.value)} className="w-full px-4 py-3 bg-gray-800 rounded" />
      <button className="w-full bg-blue-600 py-3 rounded">Send Reset Link</button>
    </form>
  );
}
...
export default function AuthClient() {
  ...
  const rawBill   = (params.get('billing') as 'monthly'|'yearly') || 'monthly';

  const [tvUser,   setTvUser]   = useState('');
  const [email,    setEmail]    = useState('');
  const [password, setPassword] = useState('');
  const [plan,     setPlan]     = useState<PlanKey>(rawPlan);
  const [billing,  setBilling]  = useState<'monthly'|'yearly'>(rawBill);
  ...
  async function handleSignup(e: FormEvent) {
    ...
    try {
      ...
      res = await fetch('/api/save-user', {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ email, tradingViewUsername: tvUser }),
      });
      if (!res.ok) throw new Error('Saving user info failed');

      const stripe = await fetch('/api/create-stripe-session', {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ plan, billing, email }),
      });
      const { url } = await stripe.json();
  ...
  return (
    <div className="min-h-screen bg-black text-white flex items-center justify-center p-6">
      <div className="max-w-lg w-full space-y-8">
        ...
        {view==='signup' && (
          <form onSubmit={handleSignup} className="space-y-6 bg-gray-900 p-6 rounded-lg">
            <button
              type="button"
            ...
            <button
              ...
              className="w-full bg-gradient-to-r from-blue-500 to-teal-500 py-3 rounded-lg"
            >
